...
export class TldrawDurableObject extends DurableObject<Env> {
	...
	private async runAutonomousAgent() {
		...
		try {
			...
			const hasWork =
				result.shouldContribute && (canvasToApply.length > 0 || !!mediaAction);
			...
			const webhookUrl = await this.ctx.storage.get<string>("agent:webhookUrl");
	...
	private async enqueueAutonomousMediaGeneration(
		...
		try {
			const renderStatus =
				targetMedia === "video" ? "Rendering video..." : "Rendering image...";
			...
			const earlyWebhook = await this.ctx.storage.get<HiggsfieldWebhookPayload>(
			...
			if (earlyWebhook) {
				...
				this.ctx.waitUntil(this.pollForCompletion(generationId, requestId));
			}
		} catch (e) {
	...
	private async applyAgentActionsToStore(
		...
		const existingRecords = this.room
			.getCurrentSnapshot()
			.documents.map((d) => d.state as unknown as Record<string, unknown>);
			...
			for (const action of actions) {
				switch (action.type) {
					...
					case "group": {
						...
						const maxX = Math.max(
							...groupShapes.map(
								(s) => s.x + (typeof s.props?.w === "number" ? s.props.w : 200),
							),
						);
						const maxY = Math.max(
							...groupShapes.map(
								(s) => s.y + (typeof s.props?.h === "number" ? s.props.h : 200),
							),
						);
						const padding = 80;
	...
	private readonly router = AutoRouter({ catch: (e) => error(e) })
		...
		.post("/api/agent/reorganize", (request) => this.handleReorganize(request))
		.post("/api/agent/set-mode", (request) => this.handleSetAgentMode(request))
		...
		.get("/api/snapshots/list", (request) => this.handleListSnapshots(request))
	...
	private broadcast(data: object) {
		for (const session of this.room.getSessions()) {
			try {
				...
				console.warn("[broadcast] failed for session", session.sessionId, e);
			}
		}
	}
	...
	private async replayGenerations(sessionId: string) {
		...
		for (const gen of generations) {
			try {
				...
				} else if (gen.status === "working" || gen.status === "submitted") {
					this.room.sendCustomMessage(sessionId, {
						type: "agent:status",
						generationId: gen.generationId,
	...
	private async finalizeGeneration(
		...
		if (payload.status === "completed") {
			...
			// const mediaType = gen.mediaType ?? (videoUrl ? "video" : "image");
			const mediaUrl =
				mediaType === "video" ? (videoUrl ?? imageUrl) : (imageUrl ?? videoUrl);
		...
		for (let i = 0; i < MAX_POLLS; i++) {
			...
			try {
				...
				if (
					...
					console.log("[poll] generation complete, finalizing:", generationId);
	...
	private async handleHiggsfieldWebhook(request: IRequest) {
		...
		if (!generationId) {
			...
			await this.ctx.storage.put(this.webhookQueueKey(requestId), payload);
			return Response.json({ ok: true });
		}
	...
	private async runVoiceCommandPipeline(command: string, webhookUrl?: string) {
		const generationId = crypto.randomUUID();
		const createdAt = Date.now();
		console.log(
		...
		try {
			...
			if (hasImageReference) {
				...
				const { imageBase64, mimeType } = await this.fetchCanvasImageAsBase64(
					refImage.src,
				);

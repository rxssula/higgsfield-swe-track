...
type SignalMessage =
	...
	| { type: 'voice-offer'; from: string; to: string; offer: RTCSessionDescriptionInit }
	| { type: 'voice-answer'; from: string; to: string; answer: RTCSessionDescriptionInit }
		...
		rec.onresult = (event) => {
			for (let i = event.resultIndex; i < event.results.length; i++) {
				...
				if (command && this.onCommand) {
					console.log(`[voice-trigger] "${this.triggerWord}" → "${command}"`)
